...
{capture name="thumb_default_width"}{getWidthSize type='thumb_default'}{/capture}
{capture name="thumb_default_height"}{getHeightSize type='thumb_default'}{/capture}
            ...
            <ul class="pro_column_list">
                {foreach from=$new_products item=newproduct name=myLoop}
                    <li class="pro_column_box clearfix">
                        <a class="pro_column_left products-block-image" href="{$newproduct.link|escape:'html'}" title="{$newproduct.legend|escape:html:'UTF-8'}"><img class="replace-2x img-responsive" src="{$link->getImageLink($newproduct.link_rewrite, $newproduct.id_image, 'thumb_default')|escape:'html'}" alt="{$newproduct.name|escape:html:'UTF-8'}" width="{$smarty.capture.thumb_default_width}" height="{$smarty.capture.thumb_default_height}" /></a>
