...
<div class="addresses clearfix">
	...
	{else}
		<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn btn-default">{l s='Add a new address'}</a>
	{/if}
	</p>
	<div class="row">
	...
	<p class="address_add submit">
		<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back|urlencode}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn btn-default">{l s='Add a new address'}</a>
	</p>
	{if !$opc}
...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				{if $back}
					<a href="{$link->getPageLink('order', true, NULL, "back={$back}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn btn-default pull-left">{l s='Continue Shopping'}</a>
				{else}
					<a href="{$link->getPageLink('order', true, NULL)|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn btn-default pull-left">{l s='Continue Shopping'}</a>
				{/if}
		        <button type="submit" name="processAddress" class="btn btn-default pull-right">{l s='Proceed to checkout'}</button>
			</p>
		</form>
{else}
	</div>
