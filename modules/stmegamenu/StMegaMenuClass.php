...
class StMegaMenuClass extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields' => array(
            ...
            'nofollow'        => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
    ...
    public static function getSub($id_parent,$active=0,$id_lang,$custom_content)
    {
        return Db::getInstance()->executeS('
            ...
            ($custom_content==1 ? ' AND smm.`spans`=0 ' : ($custom_content==2 ? ' AND smm.`spans`>0 ' : ' AND (smm.`spans`=0 OR (smm.`spans`>0 AND (smml.`html`!="" || smm.`id_product`>0 || smm.`id_st_blog`>0)))')).
            ' '.Shop::addSqlRestrictionOnLang('smm').'
    ...
    public static function recurseTree($id_parent,$max_depth=2,$current_depth=0,$active=0,$id_lang = null,$custom_content=0)
    {
        ...
        if (($current_depth < $max_depth) && $tree && count($tree))
            foreach($tree as &$v)
            {
                ...
                {
                    $category = new Category($v['id_category'], $id_lang);
                ...
                elseif($megamenu->stblog_status && $v['id_st_blog_category'] && isset($v['auto_sub']) && $v['auto_sub'])
                {
                    $blog_category = new StBlogCategory((int)$v['id_st_blog_category'],$id_lang);
                    $sub_categories = $blog_category->recurseLiteCategTree(2,$current_depth);
                    if(is_array($sub_categories['children']) && count($sub_categories['children']))
                    {
                        $clone_v = $v;
                        unset($clone_v['id_st_mega_menu'],$clone_v['id_parent']);
                        $clone_v['style'] = 0;
                        $v['children'] = self::recurseCategories($clone_v,$sub_categories['children'],$current_depth+1, 'id_st_blog_category');
                    }
                    $custom_content_child = self::getSub($v['id_st_mega_menu'],$active,$id_lang,2);
                    if(is_array($custom_content_child) && count($custom_content_child))
                    {
                        foreach($custom_content_child as $m => $n)
                            if($n['html']=='' && !$n['id_st_blog'])
                                unset($custom_content_child[$m]);                                
                        $v['children'] = isset($v['children']) ? array_merge($v['children'],$custom_content_child) : $custom_content_child;
                    }
                }
    ...
    public static function recurseCategories($parent,$sub_categories,$current_depth,$identifer = null)
    {
        $tree = array();
        foreach($sub_categories as $v)
        {
            $tmp = $parent;
            $tmp[$identifer ? $identifer : 'id_category'] = $v['id'];
            $tmp['level_depth'] = $current_depth;
            if(is_array($v['children']) && count($v['children']))
                $tmp['children'] = self::recurseCategories($parent,$v['children'], $current_depth+1, $identifer);
            $tree[] = $tmp;
        }
        return $tree;
    }
