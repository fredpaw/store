...
{
    protected static $cache_meum_items;
	private $_html = '';
    public $fields_list;
    public $fields_form;
    private $_baseUrl;
    ...
    public function sampleData($id_shop)
    {
		...
		$samples = array(
			array('id_st_mega_menu' => '', 'id_parent' => 0, 'level_depth' => 0, 'style' => 1, 'id_icon' => 1, 'span' => 0, 'title' => '', 'link' => ''),
			array('id_st_mega_menu' => '','id_parent' => 0, 'level_depth' => 0, 'style' => 15, 'id_icon' => 0, 'span' => 0, 'title' => 'Custom block', 'link' => '', 'child' => array(
                array('level_depth' => 1, 'spans' => 4, 'html' => '<h4 class="ma_level_1">Transformer</h4><p>Transformer is an elegant, powerful and fully responsive prestashop theme with modern design. Suitable for every type of store.</p><ol><li>Fully responsive theme</li><li>Unlimited colors</li><li>Customizable design</li><li>Powerfull theme editor</li><li>Built with HTML5 and CSS3</li><li>Google fonts</li><li>Ajax Wishlist and Compare</li></ol>'),
		...
		foreach($samples as $k=>&$sample)
		{
			...
			foreach (Language::getLanguages(false) as $lang)
            {
				...
				$module->link[$lang['id_lang']] = $sample['link'];
            }
	...
	{            
		...
		if (isset($_POST['savemenustmegamenu']) || isset($_POST['savemenustmegamenuAndStay']))
        {
            if($id_st_mega_menu)
				$menu = new StMegaMenuClass($id_st_mega_menu);
			else
            ...
            if (!count($error) && $menu->validateFields(false) && $menu->validateFieldsLang(false))
            {
                if($menu->save())
                {
                    ...
                    if(isset($_POST['savemenustmegamenuAndStay']))
	    ...
	    if ((Tools::isSubmit('activestmegamenu')))
        {
            ...
            if(Validate::isLoadedObject($menu) && $menu->toggleStatus())
            {
                ...
                if($menu->id_parent)
                {
                    $jump_id = $menu->id_parent;
                    if($menu->level_depth>1)
                        $jump_id = StMegaMenuClass::getSecondaryParent($jump_id);
                    Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_mega_menu='.$jump_id.'&view'.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'));
                }
                else
        ...
        elseif(Tools::isSubmit('viewstmegamenu'))
        {
            ...
            if($menu->auto_sub && isset(self::$_layout[$menu->style]) && !count(self::$_layout[$menu->style][1]))
                    Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_mega_menu='.$id_st_mega_menu.'&update'.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'));
            
            if(($menu->id_category || $menu->id_st_blog_category) && $menu->auto_sub)
            {
        ...
        else
        {
            ...
            $list = array();
            $list_skip_actions['view'] = array();
            if($all)
                $list = $this->_toFlat($all);
            foreach($list AS $row)
                if ($row['auto_sub'] && isset(self::$_layout[$row['style']]) && !count(self::$_layout[$row['style']][1]))
                    $list_skip_actions['view'][] = $row['id_st_mega_menu'];
            $helper->list_skip_actions = $list_skip_actions;
    ...
    private function _toFlat($arr)
    {
        ...
        foreach($arr as $v)
        {
            ...
            if(($v['id_category'] || $v['id_st_blog_category']) && $v['auto_sub'])
            {
	...
	private function _displayForm()
    {
		...
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    ...
                    'desc' => $this->l('Only for product categories and blog categories.'),
				),
                'title' => array(
					'type' => 'text',
					'label' => $this->l('Title:'),
                ...
                array(
                    ...
                    'id'   => 'style',
                    'label' => $this->l('Style:'),
					'name' => $this->BuildRadioUI($this->_style, 'style', $menu->style?$menu->style:1),                    
                    'desc' => $this->l('You can put custom content to black areas'),
                ),
                array(
					'type' => 'html',
                    'id'   => 'alignment',
        ...
        if(Validate::isLoadedObject($menu))
        {
            ...
            if($type!=7)
            {
                ...
                $helper->tpl_vars['fields_value']['links'] = $type.'_'.$menu->$field;
            }
            else
                $helper->tpl_vars['fields_value']['links'] = '';
        }
		return $helper;
    }
    public function getParentList($id_parent)
    {
        ...
        {
            if(($parent['id_category'] || $parent['id_st_blog_category']) && $parent['auto_sub'])
                continue;
	...
	{                
		...
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    ...
                    'id' => 'a_cancel',
					'label' => '',
					'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_st_mega_menu='.$id_parent.'&view'.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> Back to list</a>',                  
				),
			),
            'buttons' => array(
                array(
        ...
        if($menu->id)
        {
            ...
            $helper->tpl_vars['fields_value']['id_parent'] = $id_parent;
	...
	{               
		...
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
                ...
                array(
                    ...
                    'id' => 'a_cancel',
					'label' => '',
					'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_st_mega_menu='.$menu->id_parent.'&view'.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> Back to list</a>',                  
				),
			),
            'buttons' => array(
                array(
        ...
        if($menu->id_product)
        {
            $helper->tpl_vars['fields_value']['content'] = 1;
        }
        else
        {
            $this->fields_form[0]['form']['input']['product_name']['disabled'] = true;
            $helper->tpl_vars['fields_value']['id_product'] = 0;
            $helper->tpl_vars['fields_value']['content'] = 0;
        }
		return $helper;
	}
    ...
    public function initMenuList()
    {
        ...
        $helper->toolbar_btn['edit'] =  array(
			'href' => AdminController::$currentIndex.'&configure='.$this->name.'&update'.$this->name.'&id_st_mega_menu='.(int)Tools::getValue('id_st_mega_menu').'&fr=view&token='.Tools::getAdminTokenLite('AdminModules'),
			'desc' => $this->l('Edit tab'),
		);
		$helper->toolbar_btn['back'] =  array(
			'href' => AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'),
			'desc' => $this->l('Back to list')
        ...
        if (!$this->isCached('header.tpl', $this->stGetCacheId()))
        {
            ...
            if(is_array($data) && count($data))
                foreach($data as $v)
                {
                    ...
                    if($v['tab_content_bg'])
                        $custom_css .= '#st_menu_'.$v['id_st_mega_menu'].' .stmenu_sub, #st_menu_'.$v['id_st_mega_menu'].' .stmenu_sub .sttlevel2{background-color:'.$v['tab_content_bg'].';border-color:'.$v['tab_content_bg'].';}';
                }
    ...
    {
        return $this->display(__FILE__, 'stmegamenu-mobile-tri.tpl');
    }
    ...
    {
        if (!$this->isCached('stmegamenu-mobile.tpl', $this->getCacheId()))
        {
            $this->_prepareHook();            
        }
        return $this->display(__FILE__, 'stmegamenu-mobile.tpl', $this->getCacheId());
    }
    ...
    public function hookDisplayTopSecondary($param)
	{
		...
		if (!$this->isCached('stmegamenu.tpl', $this->stGetCacheId($cache_name)))
        {
            $this->_prepareHook();
        }
		return $this->display(__FILE__, 'stmegamenu.tpl', $this->stGetCacheId($cache_name));
	}
    private function _prepareHook()
    {
        $menu_pc = $menu_mob = '';
        if (!isset(StMegaMenu::$cache_meum_items))
            StMegaMenu::$cache_meum_items = StMegaMenuClass::recurseTree(0,2,0,1,$this->context->language->id,0);
            
        if(StMegaMenu::$cache_meum_items)
        {
            $menu_pc .= '<ul id="st_mega_menu" class="visible-md visible-lg clearfix">';
            $menu_mob .= '<ul id="stmobilemenu">';
            foreach(StMegaMenu::$cache_meum_items as $v)
            {
                $link = self::getLink($v,0,(isset($v['children']) && $v['children'] && count($v['children'])));
                if(!$link)
                    continue;
                $menu_pc .= '<li id="st_menu_'.$v['id_st_mega_menu'].'" class="sttlevel0 '.(isset($v['alignment']) ? ' m_alignment_'.$v['alignment'].' ' : 'm_alignment_0').'">'.$link;
                $menu_mob_tmp = $menu_mob_cate_thumb = '';
            
                if(isset($v['children']) && is_array($v['children']) && count($v['children']))
                {
                    if($v['style']==10)
                    {
                        $menu_pc .= '<div class="stmenu_sub style_classic col-md-3" >';
                        $sub_menu = array();
                        ...
                        if(count($sub_menu))
                        {
                            $menu_pc .= '<div class="row"><div class="stmenu_container">';
                    ...
                    else
                    {
                        ...
                        if((isset($v['alignment']) && $v['alignment']==2))
                            $menu_pc .= '<div class="stmenu_sub style_wide px-col-md-'.$pc_sum.' px-col-lg-'.$pc_sum.'" >';
                        else
                            $menu_pc .= '<div class="stmenu_sub style_wide col-md-'.( (isset($v['alignment']) && $v['alignment']) ? $pc_sum : '12' ).'" >';
                        ...
                        if(count(self::$_layout[$v['style']][0]) && $count_menu)
                        {
                            ...
                            $menu_pc_tmp .= '<div class="col-md-'.$span_adjust.' stmenu_container">';
                            $menu_mob_tmp .= '<span class="opener">&nbsp;</span>';
                        ...
                        if(count(self::$_layout[$v['style']][1]) && $count_html)
                        {
                            ...
                            $menu_pc_tmp .= '<div class="col-md-'.$span_adjust.'"><div class="row">';
                        ...
                        if($pc_sum)
                            $menu_pc .= '<div class="row">'.$menu_pc_tmp.'</div>';
                        ...
                        if($v['id_category'])
                        {
                            ...
                            if (count($files) > 0)
                            {
                                $category = new Category((int)$v['id_category'],$this->context->language->id);

                                $menu_pc .= '<div class="row st_cate_thumbnail">';
                                $menu_mob_cate_thumb .= '<div class="stm_cate_thumbnail">';
                                $col_nbr_sum = 0;
                                foreach ($files as $file)
                                    if (preg_match('/'.$v['id_category'].'-([0-9])?_thumb.jpg/i', $file) === 1)
                                    {
                                        $cate_thumb = $this->context->link->getMediaLink(_THEME_CAT_DIR_.$file);

                                        $col_nbr = ($v['style']==10 || $pc_sum<=4) ? 12 : ( ($pc_sum == 12 || !isset($v['alignment']) || !$v['alignment']) ? 4 : 6 );
                                        $col_nbr_sum += $col_nbr;

                                        $menu_pc .= '<div class="col-md-'.$col_nbr.'"><img src="'.$cate_thumb
                                        .'" alt="'.Tools::SafeOutput($category->name).'" title="'
                                        .Tools::SafeOutput($category->name).'" class="imgm" /></div>';

                                        $menu_mob_cate_thumb .= '<div class="stm_cate_thumb"><img src="'.$cate_thumb
                                        .'" alt="'.Tools::SafeOutput($category->name).'" title="'
                                        .Tools::SafeOutput($category->name).'" class="imgm" /></div>';

                                        if($col_nbr_sum==12)
                                        {
                                            $menu_pc .= '</div><div class="row st_cate_thumbnail mar_t1">';
                                            $col_nbr_sum = 0;
                                        }
                                    }
                                $menu_pc .= '</div>';
                                $menu_mob_cate_thumb .= '</div>';
                            }
                        }
                ...
                if(!isset($v['hide_on_mobile']) || (isset($v['hide_on_mobile']) && !$v['hide_on_mobile']))
                    $menu_mob .= '<li class="stmlevel0">'.self::getLink($v,0).$menu_mob_tmp.$menu_mob_cate_thumb.'</li>';
            }
            $menu_pc .= '</ul>';
            $menu_mob .= '</ul>';
        }
    ...
    public function buildHtml($tree,&$pc,&$mob,$map)
    {
        if(is_array($tree) && count($tree))
        {
            ...
            foreach($tree as $v)
            {
                if(!$v['spans'])
                    continue;
                $pc .= '<div class="col-md-'.$map[$v['spans']].' menu_custom_content" style="'.($v['bg_color'] ? 'background-color:'.$v['bg_color'].';' : '').'">';
	...
	private function getProductInfo($id, $i=NULL)
	{
		...
		if (Validate::isLoadedObject($product))
        {
            ...
            if ($tax_calc == 0 || $tax_calc == 2)
                $displayed_price = Product::getPriceStatic((int)$product->id, true, null);
            elseif ($tax_calc == 1)
                $displayed_price = Product::getPriceStatic((int)$product->id, false, null);
            
            $cover = Product::getCover($product->id);
            $product_link = $product->getLink();
            $homeSize = Image::getSize(ImageType::getFormatedName('home'));
            $res .= '<div class="menu_product_wrap text-center">
                <a href="'.htmlspecialchars($product_link, ENT_QUOTES, 'UTF-8').'" title="'.htmlspecialchars($product->name, ENT_COMPAT, 'UTF-8').'" class="product_image mar_b6"><img class="replace-2x" src="'.$this->context->link->getImageLink($product->link_rewrite, $product->id.'-'.$cover['id_image'], 'home_default').'" alt="'.htmlspecialchars($product->name, ENT_COMPAT, 'UTF-8').'" width="'.$homeSize['width'].'" height="'.$homeSize['width'].'" />'.((Configuration::get('STSN_NEW_STYLE')!=2 && isset($product->new) && $product->new == 1) ? '<span class="new"><i>'.$this->l('New').'</i></span>' : '').((Configuration::get('STSN_SALE_STYLE')!=2 && isset($product->on_sale) && $product->on_sale && isset($product->show_price) && $product->show_price && !Configuration::get('PS_CATALOG_MODE')) ? '<span class="on_sale"><i>'.$this->l('Sale').'</i></span>' : '').'</a>
            ...
            if(!$catalog_mod)
                $res .= '<div class="price_container"><span class="price">'.Tools::displayPrice($displayed_price).'</span>'.((isset($product->specificPrice['reduction']) && $product->specificPrice['reduction']) ? ' <span class="old_price">'.Tools::displayPrice($product->getPriceWithoutReduct($tax_calc==1 ? true : false)).'</span>' : '').'</div>';
            $res .= '</div>';
        }
        return $res;
	}
    ...
    public function buildUl($tree,&$pc,&$mob,$level=1,$style=0,$map,$timer=0)
    {
        if(is_array($tree) && count($tree))
        {
            ...
            $pc .= '<ul class="'.($level==1 ? 'row' : '').' sttlevel'.$level.'">';
            ...
            foreach($tree as $v)
            {
                ...
                if($level==1 && $timer++%$spans_nbr==0 && $timer>1)
                    $pc .= '</ul><ul class="row sttlevel'.$level.'">';
                    
                $pc .= '<li '.(isset($v['id_st_mega_menu']) ? ' id="st_megamenu_li_'.$v['id_st_mega_menu'].'" ' : '').' class="mllevel'.$level.($level==1 ? ' col-md-'.$map[$spans].' ' : '').'" style="'.( ($v['bg_color'] && isset($v['id_parent']) && $v['id_parent']) ? 'background-color:'.$v['bg_color'].';' : '').'" >'.$link; 
                $mob_menu = '<li '.(isset($v['id_st_mega_menu']) ? ' id="st_mobmenu_li_'.$v['id_st_mega_menu'].'" ' : '').'>'.$link; 
                if(isset($v['children']) && is_array($v['children']) && count($v['children']))
                {
    ...
    public function buildUlClassic($tree,&$pc,&$mob,$level=1)
    {
        if(is_array($tree) && count($tree))
        {
            ...
            foreach($tree as $v)
            {
                ...
                $pc .= '<li '.(isset($v['id_st_mega_menu']) ? ' id="st_megamenu_li_'.$v['id_st_mega_menu'].'" ' : '').' class="mllevel'.$level.'" >'.$link; 
                $mob_menu = '<li '.(isset($v['id_st_mega_menu']) ? ' id="st_mobmenu_li_'.$v['id_st_mega_menu'].'" ' : '').'>'.$link; 
                if(isset($v['children']) && is_array($v['children']) && count($v['children']))
                {
    ...
    public static function getLink($row,$level=0,$is_parent=false)
	{
        ...
        if(!$name && !$icon)
            return '';
        return '<a href="'.($link ? htmlspecialchars($link, ENT_QUOTES, 'UTF-8') : 'javascript:;').'"  style="'.(($row['txt_color'] && isset($row['id_parent']) && $row['id_parent']) ? 'color:'.$row['txt_color'].';':'').(($row['bg_color'] && isset($row['id_parent']) && $row['id_parent']) ? 'background-color:'.$row['bg_color'].';':'').'" '.($row['new_window'] ? ' target="_blank" ':'').' class="ma_level_'.$level.($is_parent ? ' is_parent ' : '').($icon ? ' is_icon_menu ' : '').'" '.((int)$row['nofollow'] > 0 ? ' rel="nofollow"' : '').(!Configuration::get('STSN_MENU_TITLE') ? ' title="'.htmlspecialchars(($title ? $title : $name), ENT_COMPAT, 'UTF-8').'"' : '').'>'.$icon.htmlspecialchars(Tools::substr($name, 0, 30), ENT_COMPAT, 'UTF-8').(Tools::strlen($name)>30 ? '...' : '').($is_parent ? '<i class="icon-down-dir-2"></i>' : '').'</a>';
	}
