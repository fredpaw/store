...
class StBlogCategory extends ObjectModel
{
    ...
    public function recurseLiteCategTree($max_depth = 3, $current_depth = 0, $id_lang = null, $excluded_ids_array = null)
	{
		...
		return array(
			'id' => (int)$this->id,
			'link' => Context::getContext()->link->getModuleLink('stblog','category',array('blog_id_category'=>$this->id,'rewrite'=>$this->link_rewrite)),
			'name' => $this->name,
			'children' => $children
		);
	}
	...
	public function getLink(Link $link = null)
	{
		...
		return $link->getStBlogCategoryLink($this->id, $this->link_rewrite);
	}
    ...
    public static function getCategoryMapInfo($id_blog= 0)
    {
    ...
    public static function getCategoryMapIds($id_blog = 0)
    {
        $rs = Db::getInstance()->executeS('
        ...
        WHERE cb.id_st_blog = '.(int)$id_blog.'
        ');
        $ret = array();
        foreach($rs AS $v)
