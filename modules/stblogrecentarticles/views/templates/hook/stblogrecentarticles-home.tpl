...
<div id="st_blog_recent_article_container_{$hook_hash}" class="st_blog_recent_article_container block {if $hide_mob} hidden-xs {/if}">
{if isset($isHomeVeryBottom) && $isHomeVeryBottom}<div class="wide_container"><div class="container">{/if}
<section id="st_blog_recent_article{$hook_hash}" class="st_blog_recent_article section">
	<h3 class="title_block"><span>{l s='Recent articles ' mod='stblogrecentarticles'}</span></h3>
    <script type="text/javascript">
    //<![CDATA[
    var recent_article_itemslider_options{$hook_hash};
    //]]>
    </script>
    {if is_array($blogs) && $blogs|count}
    {if !isset($display_as_grid) || !$display_as_grid}
        <div id="recent_article_itemslider-{$hook_hash}" class="recent_article_itemslider flexslider">
        ...
        {literal}
        jQuery(function($) {
            recent_article_itemslider_options{/literal}{$hook_hash}{literal} = {
                ...
                controlNav: false,
                controlsContainer: "#recent_article_itemslider-{/literal}{$hook_hash} {if !isset($isbloghomepage) && isset($direction_nav) && $direction_nav}.nav_left_right{else}.nav_top_right{/if}{literal}",
                itemWidth: 260,
            ...
            $('#recent_article_itemslider-{/literal}{$hook_hash}{literal} .sliderwrap').flexslider(recent_article_itemslider_options{/literal}{$hook_hash}{literal});
            
            var recent_article_itemslider_rs{/literal}{$hook_hash}{literal};
            $(window).resize(function(){
                clearTimeout(recent_article_itemslider_rs{/literal}{$hook_hash}{literal});
                var rand_s = parseInt(Math.random()*200 + 300);
                recent_article_itemslider_rs{/literal}{$hook_hash}{literal} = setTimeout(function() {
                    {/literal}{if isset($column_slider) && $column_slider}{literal}
                    ...
                    {/literal}{/if}{literal}
                    var flexslide_object = $('#recent_article_itemslider-{/literal}{$hook_hash}{literal} .sliderwrap').data('flexslider');
