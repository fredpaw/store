...
<div id="product-images" class="panel product-tab">
	...
	<table class="table tableDnD" id="imageTable">
		<thead>
			<tr class="nodrag nodrop"> 
                ...
                {if isset($show_hover) && $show_hover}<th class="fixed-width-xs"><span class="title_box">{l s='Hover'}</span></th>{/if}
				<th></th> <!-- action -->
			</tr>
		</thead>
		<tbody id="imageList">
	...
	<table id="lineType" style="display:none;">
		<tr id="image_id">
            ...
            {if isset($show_hover) && $show_hover}
            <td class="hover">
				<a href="#">
		...
		$(document).ready(function(){
			...
			{foreach from=$images item=image}
				...
				if (assoc != {literal}"{"{/literal})
				{
					...
					assoc = false;
				imageLine({$image->id}, "{$image->getExistingImgPath()}", {$image->position}, "{if $image->cover}icon-check-sign{else}icon-check-empty{/if}", assoc, "{$image->legend[$default_language]|@addcslashes:'\"'}","{if $image->hover}icon-check-square{else}icon-square-o{/if}");
			{/foreach}
			{literal}
            ...
            {/literal}{if isset($show_hover) && $show_hover}{literal}
            $('i.hover').die().live('click', function(e)
			{
                ...
                $.ajax({
                    url: '{/literal}{$ajax_link_hover}{literal}',
                    type: 'get',
                    dataType: 'json',
                    data: {
                        'id_image' : id,
