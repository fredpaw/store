...
class StAdvancedMenuClass extends ObjectModel
{
	/** @var integer banner id*/
    public $id;
	public $location;
    public $id_parent;
	public $id_st_advanced_column;
	public $id_shop;
    public $level_depth;
    public $item_k;
    public $item_v;
	...
	public static $definition = array(
		'table'     => 'st_advanced_menu',
		'primary'   => 'id_st_advanced_menu',
		'multilang' => true,
		'fields' => array(
            'location'       => array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt'),
            'id_st_advanced_column'       => array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt'),
    ...
    public function deleteRec($sub)
    {
        foreach($sub as $v)
        {
			...
			DELETE FROM `'._DB_PREFIX_.'st_advanced_menu`
			WHERE `id_st_advanced_menu`='.(int)$v['id_st_advanced_menu']);
            Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'st_advanced_menu_lang`
			WHERE `id_st_advanced_menu`='.(int)$v['id_st_advanced_menu']);
            
            $this->clearExtraItems($v['id_st_advanced_menu']);
        }
        
    }
    
    public function clearExtraItems($id_advanced_menu = 0)
    {
        if ($id_advanced_menu > 0)
        {
            StAdvancedColumnClass::deleteByMenu((int)$id_advanced_menu);
            StAdvancedProductClass::deleteMenuProducts((int)$id_advanced_menu);
            StAdvancedBrandClass::deleteByMenu((int)$id_advanced_menu);
        }
    }
    ...
    public static function justifyTree(&$tree, &$n)
	{
		...
		foreach ($tree as $v)
		{
            ...
            $right = (int)$n++;
    		Db::getInstance()->execute('
    			UPDATE '._DB_PREFIX_.'st_advanced_menu
    			SET nleft = '.(int)$left.', nright = '.(int)$right.'
    			WHERE id_st_advanced_menu = '.(int)$v['id_st_advanced_menu']);
	    }
	}
    
    public static function getById($id_st_advanced_menu,$id_lang)
    {
        return Db::getInstance()->getRow('
            SELECT smm.*,smml.`html`,smml.`title`,smml.`link`,smml.`cate_label`
            FROM `'._DB_PREFIX_.'st_advanced_menu` smm
            LEFT JOIN `'._DB_PREFIX_.'st_advanced_menu_lang` smml ON smm.`id_st_advanced_menu`=smml.`id_st_advanced_menu`
            WHERE smml.`id_lang`='.(int)$id_lang.
            ' AND smm.`id_st_advanced_menu`='.(int)$id_st_advanced_menu.
    ...
    public static function getByColumnId($id_st_advanced_column, $id_lang, $active=0,$item_t=0,$id_parent=0)
    {
        $res = Db::getInstance()->executeS('
            ...
            LEFT JOIN `'._DB_PREFIX_.'st_advanced_menu_lang` smml ON smm.`id_st_advanced_menu`=smml.`id_st_advanced_menu`
            WHERE smml.`id_lang`='.(int)$id_lang.
            ' AND smm.`id_st_advanced_column`='.(int)$id_st_advanced_column.
    ...
    public static function recurseTree($id_parent,$max_depth=2,$current_depth=0,$active=0,$id_lang = null,$item_t=0)
    {
        ...
        if ( ( $max_depth==0 || ($current_depth+1 < $max_depth) ) && $tree && count($tree))
            foreach($tree as &$v)
            {
                $jon = self::recurseTree($v['id_st_advanced_menu'],$max_depth,$current_depth+1,$active,$id_lang,$item_t);
    ...
    public static function getTypes()
    {
		...
		return array(
			1 => $module->l('Category', 'StAdvancedMenu'),
			2 => $module->l('Product', 'StAdvancedMenu'),
			3 => $module->l('CMS', 'StAdvancedMenu'),
			4 => $module->l('Manufacturer', 'StAdvancedMenu'),
			5 => $module->l('Supplier', 'StAdvancedMenu'),
			6 => $module->l('Shop', 'StAdvancedMenu'),
			7 => $module->l('Link', 'StAdvancedMenu'),
			8 => $module->l('CMS category', 'StAdvancedMenu'),
			9 => $module->l('ICON', 'StAdvancedMenu'),
			10 => $module->l('Blog category', 'StAdvancedMenu'),
			11 => $module->l('Blog', 'StAdvancedMenu'),
			12 => $module->l('Permanent link', 'StAdvancedMenu'),
		);
    }
    public static function getTopParent($id_st_advanced_menu)
    {
        $menu = new StAdvancedMenuClass($id_st_advanced_menu);
        if($menu->id_parent)
            return StAdvancedMenuClass::getTopParent($menu->id_parent);
        else
            return $id_st_advanced_menu;
    }
    public static function getSecondaryParent($id_st_advanced_menu)
    {
        $menu = new StAdvancedMenuClass($id_st_advanced_menu);
        if($menu->level_depth > 0)
            return StAdvancedMenuClass::getSecondaryParent($menu->id_parent);
        else
            return $id_st_advanced_menu;
    }
    ...
    public static function deleteByColumn($id_st_advanced_column=0)
    {
        if (!$id_st_advanced_column)
            return false;
        $res = Db::getInstance()->executeS('
            SELECT `id_st_advanced_menu`
            FROM `'._DB_PREFIX_.'st_advanced_menu`
            WHERE `id_st_advanced_column` = '.(int)$id_st_advanced_column.'
        ');
        $ret = true;
