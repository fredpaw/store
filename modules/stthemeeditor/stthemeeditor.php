...
{	
	...
	public function __construct()
	{
        ...
        $this->defaults = array(
    		...
    		"font_heading"                  => array('exp'=>1,'val'=> 'Fjalla One'),
    ...
    public function initFieldsForm()
    {
        ...
        $this->fields_form[1]['form'] = array(
			'input' => array(
                ...
                array(
					'type' => 'switch',
					'label' => $this->l('Show category descriptions on category pages:'),
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->l('Show full category descriptions on category pages:'),
    ...
    protected function initForm()
	{
        ...
        foreach (array('font_text'=>3, 'font_heading'=>3, 'font_price'=>3, 'font_menu'=>5, 'adv_font_menu'=>20, 'c_menu_font'=>21, 'font_cart_btn'=>3) as $font=>$wf) {
    ...
    public function writeCss()
    {
        ...
        $fontText = $fontHeading = $fontPrice = $fontMenu = $fontAdvancedMenu = $fontCMenu = $fontCartBtn = '';
        $fontTextWeight = $fontHeadingWeight = $fontPriceWeight = $fontMenuWeight = $fontAdvancedMenuWeight = $fontCMenuWeight = $fontCartBtnWeight = '';
        $fontTextStyle = $fontHeadingStyle = $fontPriceStyle = $fontMenuStyle = $fontAdvancedMenuStyle = $fontCMenuStyle = $fontCartBtnStyle = '';
        ...
        if($fontAdvancedMenuString = Configuration::get('STSN_ADV_FONT_MENU'))
        {
            ...
            $fontAdvancedMenuArr[2] && $fontAdvancedMenuWeight = 'font-weight:'.$fontAdvancedMenuArr[2][0].';';
            $fontAdvancedMenuArr[3] && $fontAdvancedMenuStyle = 'font-style:'.$fontAdvancedMenuArr[3][0].';';
        }
        if($fontCMenuString = Configuration::get('STSN_C_MENU_FONT'))
        {
            ...
            $fontCMenuArr[2] && $fontCMenuWeight = 'font-weight:'.$fontCMenuArr[2][0].';';
            $fontCMenuArr[3] && $fontCMenuStyle = 'font-style:'.$fontCMenuArr[3][0].';';
        }
        ...
        if($fontAdvancedMenu && $fontAdvancedMenu != $fontText)
        {
            $css_font_advanced_menu .= 'font-family: "'.$fontAdvancedMenu.'";'.$fontAdvancedMenuWeight.$fontAdvancedMenuStyle;
            $css_font_mobile_advanced_menu .= 'font-family: "'.$fontAdvancedMenu.'";';
            $css .= '.advanced_style_wide .advanced_ma_level_1{font-family: "'.$fontAdvancedMenu.'";'.$fontAdvancedMenuWeight.$fontAdvancedMenuStyle.'}';
        }
        ...
        //    
        $css .= '.block .title_block, .block a.title_block, .block .title_block a, .idTabs a,.product_accordion_title,.heading,.page-heading,.page-subheading,.pc_slider_tabs a, #home-page-tabs li a, #home-page-tabs li span{'.$css_font_heading.$css_font_heading_size.'}';
        ...
        //advanced menu
        $css .= '#st_advanced_menu_wrap .advanced_ma_level_0{'.$css_font_advanced_menu.'}'; 
        ...
        if($fontCMenu && $fontCMenu != $fontText)
        {
            $css_font_c_menu .= 'font-family: "'.$fontCMenu.'";'.$fontCMenuWeight.$fontCMenuStyle;
        }
    ...
    background: linear-gradient(top, rgb('.$breadcrumb_bg_star.'), rgb('.$breadcrumb_bg_end.') 5%, rgb('.$breadcrumb_bg_end.') 95%, rgb('.$breadcrumb_bg_star.'));
        ...
        if($header_topbar_bg = Configuration::get('STSN_HEADER_TOPBAR_BG'))
        {
            ...
            if($header_topbar_opacity>=0 && $header_topbar_opacity<1)
            {
                ...
                $css .= 'body#index #page_header.transparent_header #top_bar{background:rgba('.$header_topbar_bg_hex[0].','.$header_topbar_bg_hex[1].','.$header_topbar_bg_hex[2].','.$header_topbar_opacity.');}';      
                $css .= 'body#index.mobile_device #page_header.transparent_header #top_bar{background-color:'.$header_topbar_bg.';}';
            }
        }

        if(Configuration::get('STSN_HEADER_TOPBAR_SEP'))
            $css .='#top_bar #header_user_info a, #top_bar #header_user_info span, #stsocial_list_topbar li a, #contact-link a, .shop-phone, #top_bar .dropdown_tri_inner,#top_bar .shopping_cart_style_1 .shopping_cart{border-color:'.Configuration::get('STSN_HEADER_TOPBAR_SEP').';}'; 
        ...
        if(Configuration::get('STSN_C_MENU_BG'))
            $css .='#st_advanced_menu_column_desktop .advanced_ml_level_0 .advanced_ma_level_0{background-color:'.Configuration::get('STSN_C_MENU_BG').';}'; 
        ...
        {
            $css .='#st_advanced_menu_column_desktop{border:1px solid '.$c_menu_border_color.';}'; 
            $css .='#st_advanced_menu_column_desktop .advanced_ml_level_0{border-bottom:1px solid '.$c_menu_border_color.';}#st_advanced_menu_column_desktop .advanced_ml_level_0:last-child{border-bottom:none;}'; 
        }
       ...
       if($adv_megamenu_position = Configuration::get('STSN_ADV_MEGAMENU_POSITION'))
        ...
        if(Configuration::get('STSN_CART_ICON'))
            $css .= '.icon-basket.icon_btn:before,.box-info-product .exclusive:before{ content: "\\'.dechex(Configuration::get('STSN_CART_ICON')).'"; }';
